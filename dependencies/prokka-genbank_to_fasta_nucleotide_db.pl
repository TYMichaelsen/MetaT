...
while (my $seq = $in->next_seq) {
  ...
  for my $f ($seq->get_SeqFeatures)   {
    ...
    #$cds = $cds->translate(-codontable_id => $tt, -complete => 1);			### EDITED ###			
    
    my $ec = TAG($f, 'EC_number') || $blank;
    my $gene = TAG($f, 'gene') || $blank;
    my $function = TAG($f, 'function') || $blank;					### EDITED ###
    my $locus_tag = TAG($f, 'locus_tag') || $blank;					### EDITED ###
    my $stuff = join '|', $prod, $locus_tag, $function;					### EDITED ###
    $cds->desc( join $sep, $ec, $gene, $stuff );				        ### EDITED ###
    $cds->display_id($id);
